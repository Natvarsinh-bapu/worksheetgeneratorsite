...
class WorksheetController extends Controller
{
    public function selectLayout(){
        ...
        return view('institute.worksheet.layout_selection', compact('categories', 'concepts', 'sub_concepts', 'types'));
    }
    ...
    public function generateWorksheet($id){
        ...
        return view('institute.worksheet.layout_main', compact('id', 'categories', 'concepts', 'sub_concepts', 'types'));
    }
    ...
    public function saveHtmlWorksheet(Request $request){        
        ...
        $imageName = $date_string . '_' . str_random(10).'.png';
        Storage::disk('public')->put('worksheet_png/'. $imageName, base64_decode($image));
    ...
    public function editableWorksheets(){
        ...
        $worksheets = HtmlWorksheet::where('created_by', 'institute')->where('created_by_id', $institute->id)->latest()->paginate(10);

        return view('institute.worksheet.edit_index', compact('worksheets'));
    }

    public function editWorksheet($id){
        $worksheet = HtmlWorksheet::find($id);

        return view('institute.worksheet.edit', compact('worksheet'));
    }

    public function removeWorksheet(Request $request){
        HtmlWorksheet::destroy($request->id);

        return redirect('institute/edit-worksheets')->with('success', 'Worksheet deleted');
    }
}
